...
export class Laptops implements OnInit {
  ...
  tipoLaptops: TipoModel | null = null;
  ...
  loadTipoYProductos(): void {
    ...
    this.tipoService.getAll().subscribe({
      next: (tipos) => {
        const Laptops = tipos.find((t) => t.tip_nom.toLowerCase() === 'laptops');
        if (!Laptops) {
          this.loading = false;
          this.cdr.detectChanges();
          return;
        ...
        this.tipoLaptops = Laptops;

        this.productoService.getByTipo(Laptops.tip_id).subscribe({
          next: (productos) => {
