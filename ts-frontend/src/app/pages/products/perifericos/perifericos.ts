...
export class Perifericos implements OnInit {
  ...
  marcas: MarcaModel[] = [];
  tipoperifericos: TipoModel | null = null;
  ...
  loadTipoYProductos(): void {
    ...
    this.tipoService.getAll().subscribe({
      next: (tipos) => {
        const perifericos = tipos.find((t) => t.tip_nom.toLowerCase() === 'perifericos');
        if (perifericos) {
          this.tipoperifericos = perifericos;
          this.productoService.getByTipo(perifericos.tip_id).subscribe({
            next: (data) => {
              this.products = data;
              this.setPriceRange();
              this.loading = false;
